...
def preparer_donnees(sm, quinzaine=None):
    ...
    # Nouveaux projets : type=PROJET dont date_debut dans les 30 derniers jours
    from datetime import timedelta
    TYPES_PROJET   = {"PROJET"}
    SEUIL_NOUVEAUX = datetime.now() - timedelta(days=30)
    nouveaux_projets = []
    if not meta.empty:
        ...
        for _, row in meta.iterrows():
            ...
            if type_val not in TYPES_PROJET or not pid_val:
                continue
            # Parser date_debut
            date_debut_raw = str(row.get("date_debut", "") or "").strip()
            date_debut = None
            if date_debut_raw and date_debut_raw not in ("nan", "None", ""):
                for fmt in ("%d/%m/%Y", "%Y-%m-%d", "%d-%m-%Y"):
                    try:
                        date_debut = datetime.strptime(date_debut_raw, fmt)
                        break
                    except ValueError:
                        continue
            # Nouveau si date_debut dans les 30 derniers jours
            if date_debut and date_debut >= SEUIL_NOUVEAUX:
    ...
    ${NP.length?`<div class="card"><div class="card-title">nouveaux projets jamais renseignés (${NP.length})</div>
      <div class="proj-list" style="max-height:180px;overflow-y:auto">
        ${NP.map(np=>`<div class="proj-item" style="cursor:default">
          ...
          ${np.date_debut&&np.date_debut!=="nan"?`<span style="font-size:9px;font-family:var(--font-mono);color:var(--cyan)">début:${esc(np.date_debut)}</span>`:""}
